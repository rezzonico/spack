...
description = 'Spider hierarchical modules and print which services should be loaded to get access to a module'


def setup_parser(subparser):
    subparser.add_argument('search_string', nargs=argparse.REMAINDER,
...
def spider(parser, args):
    ...
    # Construct a dictionary of services that are needed for each of those specs
    ...
    for item in mathing_specs:
        for service in spack.modules.LmodModule.hierarchy_tokens:
            if service in item and not item is item[service]:
                required_services[item].append(item[service])
        try:
            compiler_query = item.compiler.name + '@' + str(item.compiler.version)
