...
class Llvm(Package):
    ...
    version('3.0', 'a8e5f5f1c1adebae7b4a654c376a6005', url='http://llvm.org/releases/3.0/llvm-3.0.tar.gz')  # NOQA: ignore=E501
    ...
    def install(self, spec, prefix):
        ...
        if '+clang' not in spec:
            if '+clang_extra' in spec:
                raise SpackException('The clang_extra variant requires the clang variant to be selected')  # NOQA: ignore=E501
            if '+lldb' in spec:
                raise SpackException('The lldb variant requires the clang variant to be selected')  # NOQA: ignore=E501
