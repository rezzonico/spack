...
class Lammps(Package):
    ...
    def url_for_version(self, version):
        ...
        return "https://github.com/lammps/lammps/archive/stable_{0}.tar.gz" \
            .format(vdate.strftime("%d%b%Y"))
    ...
    depends_on('voropp+pic', when='+voronoi+lib')
        ...
        with working_dir('lib/atc'):
            ...
            with open('Makefile.lammps', 'w') as fh:
                ...
                makefile = [
                    ...
                    'user-atc_SYSLIB = {0}'.format(
                        self.spec['lapack'].lapack_ld_flags),
                    'user-atc_SYSPATH = ']
                fh.write('\n'.join(makefile))

    def build_voronoi(self):
        with working_dir('lib/voronoi'):
            force_symlink(self.spec['voropp'].prefix.include, 'includelink')
            filter_file(
                r'voronoi_SYSPATH =',
                'voronoi_SYSPATH = -L{0}'.format(self.spec['voropp'].prefix.lib),  # noqa: E501
                'Makefile.lammps')
